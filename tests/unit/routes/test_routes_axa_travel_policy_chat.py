...
from routes.axa_travel_policy_chat import router  # adjust if the route file has a different name
...
def test_valid_travel_policy_chat():
    response = client.post("/axa-hr/travel-policy-chat", json=sample_chat_request())
...
def test_missing_messages_field():
    ...
    response = client.post("/axa-hr/travel-policy-chat", json={})
    assert response.status_code == 422  # Unprocessable Entity

def test_empty_messages_list():
    # Provide an empty message list
    response = client.post("/axa-hr/travel-policy-chat", json={"messages": []})

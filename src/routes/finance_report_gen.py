...
convert_to_markdown_endpoint = GC_AI_PARSERS_BASE_URL + '/api/pdf/convert-to-markdown'

# Function to convert PDF to markdown using the external API
async def convert_pdf_to_markdown(file_buffer, file_name):
    headers = {
        "Authorization": f"Bearer {GC_AI_API_KEY}",
    }
...
DEFAULT_CHUNK_SIZE = 5  # can be overridden

# Split PDF into chunks
def split_pdf(file_buffer, chunk_size=DEFAULT_CHUNK_SIZE):
    file_buffer.seek(0)
...
async def generate_financial_analysis(file: UploadFile = File(...)):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            logger.info(f'Splitting the PDF into chunks of {DEFAULT_CHUNK_SIZE}')
            chunks = split_pdf(file_buffer, chunk_size=DEFAULT_CHUNK_SIZE)
